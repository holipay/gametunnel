...
**Q: 客户端需要管理员权限吗？**
A: 是的。创建虚拟网卡需要 Windows 管理员权限（UAC 提示）。
...
**Q: 支持哪些操作系统？**
A: 服务端支持 Linux 和 OpenWrt 路由器（中高端 ARM 设备）。客户端支持 Windows 10+。
...
**Q: 双击客户端没反应 / 无法运行？**
A: 从 GitHub Releases 下载的 exe 被 Windows 标记为"来自其他计算机"，可能被静默阻止运行。解决方法：右键 `gtunnel-client.exe` → **属性** → 底部勾选 **"解除锁定"（Unblock）** → 确定，然后重新双击运行。
...
go build -o bin/gtunnel-client.exe ./cmd/client     # 需要 Windows 交叉编译
GOOS=windows GOARCH=amd64 go build -o bin/gtunnel-client.exe ./cmd/client  # 指定目标平台
