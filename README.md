...
> 需要 root 权限（创建 TUN 设备）和桌面环境（systray 依赖 gtk3）。
...
# 下载预编译二进制
tar xzf GameTunnel-Client-linux-amd64.tar.gz
cd release
...
cd gametunnel
make client-linux
sudo ./bin/gtunnel-client-linux
```

### 玩家（macOS 电脑）

> 需要 root 权限（创建 utun 设备）和 Xcode CLI Tools。

```bash
# 下载预编译二进制
tar xzf GameTunnel-Client-darwin.tar.gz

# Intel Mac
sudo ./gtunnel-client

# Apple Silicon Mac
sudo ./gtunnel-client-arm64
```
...
cd gametunnel
make client-darwin
sudo ./bin/gtunnel-client-darwin
```
...
| 平台 | TUN 设备 | 配置方式 |
|------|----------|---------|
| Windows | wintun 驱动 | `netsh` + 路由表 + metric 管理 |
| Linux | `/dev/net/tun` (ioctl) | `ip addr` / `ip route` |
| macOS | utun socket | `ifconfig` / `route` |
...
### 客户端（全平台通用）
```bash
gtunnel-client -server 1.2.3.4:4700 -name 玩家名 -room 房间ID -password secret
```
| 参数 | 默认值 | 说明 |
|------|--------|------|
...
配置文件优先级：
- **Windows**：`config.ini`（exe 同目录）> `%APPDATA%\GameTunnel\config.json`
- **Linux/macOS**：`config.ini`（可执行文件同目录）> `~/.config/GameTunnel/config.json`
...
**Q: 客户端需要管理员/root 权限吗？**
A: 是的。创建虚拟网卡需要特权：Windows 需管理员（UAC 提示），Linux/macOS 需 root（`sudo`）。
...
**Q: 支持哪些操作系统？**
A: Windows 10+、Linux（需 gtk3 桌面环境）、macOS 10.15+。
...
# 编译 Linux 客户端（需在 Linux 上编译）
make client-linux

# 编译 macOS 客户端（需在 macOS 上编译）
make client-darwin
...
- **Linux**：需 `gcc` + `libgtk-3-dev` + `libayatana-appindicator3-dev`
