...
一个玩家同时运行服务端和客户端，其他玩家直接连接主机的公网 IP。无需单独部署 VPS。
...
| `-max` | `10` | 最大玩家数 |
| `-password` | _(空)_ | 房间密码 |
| `-tcp-addr` | _(空)_ | TCP 回退监听地址（留空=禁用） |
| `-verbose` | `false` | 详细日志 |
| `-name` | 计算机名 | 玩家名称 |
| `-room` | `default` | 房间 ID |
| `-lang` | `zh` | 语言 |

配置优先级：CLI 参数 > `host.ini` > 默认值

启动后，其他玩家在客户端 `config.ini` 中设置 `server=主机公网IP:4700` 即可连接。
...
**Q: 什么是主机模式？和单独部署服务端有什么区别？**
A: 主机模式（`gtunnel-host`）将服务端和客户端合并在一个进程中运行。主机玩家既是服务器也是客户端，其他玩家直接连接主机的公网 IP。适合不想单独部署 VPS 的场景。注意：主机玩家也需要 root 权限创建虚拟网卡。
...
go build -o bin/gtunnel-host ./cmd/host              # 主机模式
