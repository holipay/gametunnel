...
支持所有基于 IP 的局域网游戏，内置 UDP 广播/组播转发（适用于依赖 UDP 广播发现、mDNS、SSDP 等的游戏）。
...
设置密码后，所有游戏数据自动使用 **ChaCha20-Poly1305** 端到端加密，并通过 **ECDH** 实现前向保密：

- **密钥派生**：HKDF-SHA256(password, info="GameTunnel:"+roomID) → 32 字节密钥
- **前向保密**：ECDH (X25519) 密钥交换，每次连接生成临时密钥对
...
> **注意**：设置密码即可获得完整保护。密码同时保护认证和加密两个层面。

### 数据包完整性

所有协议包都包含 CRC32 校验和，丢弃损坏/篡改的包。加密中转包省略 CRC（AEAD 已提供完整性保证）。
...
当 UDP 被防火墙封锁时，客户端可自动通过 TCP 连接服务器：

```bash
# 服务端启用 TCP 回退
gtunnel-server -addr :4700 -tcp-addr :4700

# 客户端自动尝试：UDP 失败后回退到 TCP
```

TCP 回退使用独立的传输桥接，对游戏协议完全透明。包含读取空闲超时，防止僵死连接。
...
玩家A (10.10.0.2)              玩家B (10.10.0.3)
  ...
  └──UDP隧道──►  公网VPS Server  ◄──UDP隧道──┘
                 (10.10.0.1)            │
                 信令 + 中转 + 广播转发   └── TCP 回退（UDP 被封锁时）
```

- 所有人获得 10.10.0.x 的虚拟IP
- UDP 广播和组播（mDNS、SSDP、UPnP）通过服务器转发给同房间所有人
- 自动尝试 UDP 打洞实现 P2P 直连
- 打洞失败则通过服务器中转（10人以内延迟可接受）
- UDP 被封锁时自动回退到 TCP 传输
- 断线自动重连（指数退避）
- 会话重绑定（Session Rebinding）优雅处理客户端 IP/端口变化
...
|------|--------|------|
| `-addr` | `:4700` | UDP 监听地址 |
| `-subnet` | `10.10.0.0/24` | 虚拟子网（CIDR，仅支持 /24） |
| `-max` | `10` | 最大玩家数 |
| `-password` | _(空)_ | 房间密码（留空=无认证无加密） |
| `-tcp-addr` | _(空)_ | TCP 回退监听地址（如 `:4700`，留空=禁用） |
| `-rooms` | `false` | 多房间模式（每个房间独立子网） |
| `-max-rooms` | `64` | 多房间模式下最大自动创建房间数 |
...
| `-verbose` | `false` | 开启详细中转日志 |
| `-log-file` | _(空)_ | 日志文件路径（留空=仅 stderr） |
| `-pprof-addr` | _(禁用)_ | pprof HTTP 地址用于运行时 profiling（如 `localhost:6060`） |
| `-version` | | 显示版本信息 |
...
| 字段 | 默认值 | 说明 |
|------|--------|------|
| `server` | _(必填)_ | 服务器地址（`host:port`，支持 IPv6 如 `[::1]:4700`） |
| `port` | `4700` | 服务器端口（若 `server` 仅填主机名时使用） |
...
| `log-file` | _(空)_ | 日志文件路径（留空=仅 stderr） |
| `verbose` | `false` | 详细日志 |
| `pprof-addr` | _(禁用)_ | pprof HTTP 地址用于运行时 profiling（如 `localhost:6061`） |

配置文件优先级：`config.ini`（exe 同目录）> `%APPDATA%\GameTunnel\config.json`（Windows）/ `~/.config/GameTunnel/config.json`（Linux）

### 配置文件完整示例

```ini
# 服务器地址（host:port，支持 IPv6 如 [::1]:4700）
server=1.2.3.4:4700
...
# pprof HTTP 地址用于运行时 profiling（留空=禁用）
#pprof-addr=
```
...
A: 可以用，但 GameTunnel 更轻量、无依赖，专门为局域网游戏优化了广播/组播转发，开箱即用。
...
**Q: 支持哪些游戏？**
A: 支持所有基于 IP 的局域网游戏。广播和组播转发（UDP broadcast、mDNS、SSDP、UPnP）已内置，适用于星际争霸、红警、帝国时代等游戏。

**Q: 支持哪些操作系统？**
A: 服务端支持 Linux 和 OpenWrt 路由器（中高端 ARM 设备）。客户端支持 Windows 10+ 和 Linux。

**Q: 数据安全吗？**
A: 设置密码后，认证（HMAC-SHA256）和数据传输（ChaCha20-Poly1305）都是端到端加密的，服务端无法解密游戏数据。
...
**Q: 客户端断线后会怎样？**
A: 客户端会自动重连（指数退避）。会话重绑定（Session Rebinding）确保即使 IP/端口变化也能无缝恢复。

**Q: 服务器支持 IPv6 吗？**
A: 支持。服务器地址支持带方括号的 IPv6，例如 `server=[2408::1]:4700`。

## 更多文档

详细技术文档见 `docs/` 目录：
- [网络分析](docs/network-analysis.md) — 深入数据流、连接建立、组件架构
- [使用场景](docs/use-cases.md) — 设计理念、协议支持矩阵、路由架构
- [分支管理指南](docs/branch-management-guide.md) — 项目工作流与 PR 规范
