...
// p2pKeepaliveInterval is how often we send keepalive packets to direct
// P2P peers to prevent NAT mapping expiry (typical UDP NAT timeout: 30-120s).
...
// p2pKeepaliveLoop sends periodic keepalive packets to direct P2P peers
// to prevent NAT mapping expiry. Without this, P2P paths silently break
// after the NAT timeout (typically 30-120 seconds).
func (t *Tunnel) p2pKeepaliveLoop(ctx context.Context) {
	...
	for {
		...
		t.mu.RLock()
		var directPeers []struct {
			addr *net.UDPAddr
			ip   net.IP
		}
		for _, peer := range t.peers {
			if peer.PublicAddr != nil && peer.DirectReach.Load() {
				directPeers = append(directPeers, struct {
					addr *net.UDPAddr
					ip   net.IP
				}{peer.PublicAddr, peer.VirtualIP})
			}
		}
		t.mu.RUnlock()

		for _, p := range directPeers {
			// Reuse hole punch packet format (our virtualIP) as keepalive
			payload := make([]byte, 4)
			copy(payload, t.virtualIP.To4())
			t.sendUDP(protocol.EncodeChecked(protocol.TypeHolePunch, payload), p.addr)
		}
	}
}
