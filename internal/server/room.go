...
type Room struct {
	...
	addrMap    map[netutil.RateKey]*Client  // client endpoint → Client
	mu         sync.RWMutex
...
// The broadcast channel never drops packets: it falls back to direct send if full.
func (r *Room) sendCheckedRawBypass(data []byte, to *net.UDPAddr) {
	r.sendQueue.sendBypass(data, to)
}
