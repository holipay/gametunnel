...
type Server struct {
	...
	lang       i18n.Lang
	startTime   time.Time
	statusToken string // random token for status page auth
	...
	// Rate limiting: per-client packet count per window (double-buffer swap)
	rateMu      sync.Mutex
	rateCount   map[rateKey]int
	rateCountAlt map[rateKey]int // alternate buffer for O(1) swap
...
func New(cfg Config) (*Server, error) {
	...
	// Generate random token for status page auth
	...
	s := &Server{
		...
		rateCountAlt: make(map[rateKey]int),
...
// StatusToken returns the status page authentication token.
func (s *Server) StatusToken() string {
...
// generateStatusToken creates a random hex token for status page auth.
func generateStatusToken() string {
	b := make([]byte, 16)
	rand.Read(b)
