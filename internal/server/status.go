...
func (s *Server) startStatusServer(ctx context.Context, addr string) {
	...
	mux.HandleFunc("/", s.statusAuthMiddleware(s.handleStatusHTML))
	mux.HandleFunc("/api/status", s.statusAuthMiddleware(s.handleStatusJSON))
...
// statusAuthMiddleware validates the status token query parameter.
// The token is generated at server start and printed in the banner.
// This prevents unauthorized access to player IPs and server info.
func (s *Server) statusAuthMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		token := r.URL.Query().Get("token")
		if token == "" || token != s.statusToken {
			w.WriteHeader(http.StatusUnauthorized)
			fmt.Fprint(w, "401 — append ?token=<your-token> to the URL\nThe token was printed in the server startup banner.")
			return
		}
		next(w, r)
	}
