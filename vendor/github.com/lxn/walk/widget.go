...
func (wb *WidgetBase) init(widget Widget) error {
	...
	if err == nil {
		// Tooltip failure is non-fatal — log and continue without tooltip support.
		// Some Windows configurations fail TTM_ADDTOOL (e.g. missing common controls).
		tt.AddTool(wb.window.(Widget))
	}
